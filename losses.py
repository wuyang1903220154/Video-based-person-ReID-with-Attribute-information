...
# 这里是使用的标签平滑交叉熵损失的过程的。是通过标签平滑化来计算出相应的损失的过程的。
class CrossEntropyLabelSmooth(nn.Module):
    """Cross entropy loss with label smoothing regularizer.
    交叉熵损失对于标签平滑化的过程的。
    Reference:
    Szegedy et al. Rethinking the Inception Architecture for Computer Vision. CVPR 2016.
    Equation: y = (1 - epsilon) * y + epsilon / K. 这里是来对应的标签平滑的公式的。

    Args:
        num_classes (int): number of classes.  这里是来对应分类数的
        epsilon (float): weight. # 这里是来对应的相应的权重的。
    """
    def __init__(self, num_classes, epsilon=0.1, use_gpu=True):
        super(CrossEntropyLabelSmooth, self).__init__()
        self.num_classes = num_classes  # 这里是来对应分类数的
        self.epsilon = epsilon # 这里是来所对应的权重的
        self.use_gpu = use_gpu  #  是否是使用gpu
    ...
    def forward(self, inputs, targets):
        ...
        targets = (1 - self.epsilon) * targets + self.epsilon / self.num_classes  # 这里是来对应的标签光滑化的过程的
        loss = (- targets * log_probs).mean(0).sum()  # 计算出相应的损失的过程的。
        return loss # 计算出相应的损失后，然后是来对相应的损失的进行返回的。
...
# 这里是对应的余弦交叉熵损失的过程的。要使用余弦的三元组的损失的过程的。
